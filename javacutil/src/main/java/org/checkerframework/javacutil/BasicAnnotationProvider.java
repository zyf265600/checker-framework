...
public class BasicAnnotationProvider implements AnnotationProvider {
     ...
     * {@inheritDoc}
     *
     * <p>This implementation returns true if the {@code @SideEffectFree} annotation is present on
     * the given method.
     */
    @Override
        ...
        for (AnnotationMirror am : annotationMirrors) {
            boolean found =
                    AnnotationUtils.areSameByName(
                            am, "org.checkerframework.dataflow.qual.SideEffectFree");
            if (found) {
                return true;
            }
        }
     ...
     * {@inheritDoc}
     *
     * <p>This implementation returns true if the {@code @Deterministic} annotation is present on
     * the given method.
     */
    @Override
    public boolean isDeterministic(ExecutableElement methodElement) {
        ...
        for (AnnotationMirror am : annotationMirrors) {
            boolean found =
                    AnnotationUtils.areSameByName(
                            am, "org.checkerframework.dataflow.qual.Deterministic");
            if (found) {
                return true;
            }
        }
