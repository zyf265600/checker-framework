...
public class RequireJavadoc {
    ...
    /**
     * Matches simple name of class/constructor/method/field, or full package name, where no
    ...
    /**
     * If true, print filenames relative to working directory. Setting this only has an effect if
     * the command-line arguments were absolute pathnames, or no command-line arguments were
     * supplied.
     */
    ...
    private void setJavaFiles(String[] args) {
        ...
        if (require_package_info) {
            for (Path javaFile : javaFiles) {
                @SuppressWarnings(
                        "nullness:assignment") // the file is not "/", so getParent() is non-null
    ...
    private enum PropertyKind {
        ...
        /**
         * Return the PropertyKind for the given method, or null if it isn't a property accessor
         * method.
         *
     ...
     *
     * <ul>
     *   <li>A trivial getter is named {@code getFoo}, {@code foo}, {@code hasFoo}, {@code isFoo},
     *       or {@code notFoo}, has no formal parameters, and has a body of the form {@code return
     *       foo} or {@code return this.foo} (except for {@code notFoo}, in which case the body is
     *       negated).
     *   <li>A trivial setter is named {@code setFoo}, has one formal parameter named {@code foo},
     *       and has a body of the form {@code this.foo = foo}.
     * </ul>
     *
     * @param md the method to check
    ...
    private class RequireJavadocVisitor extends VoidVisitorAdapter<Void> {
        ...
        private String errorString(Node node, String simpleName) {
            ...
            if (range.isPresent()) {
                ...
                return String.format(
                        ...
                        path, begin.line, begin.column, simpleName);
            } else {
        ...
        public void visit(MethodDeclaration md, Void ignore) {
            ...
            if (dont_require_trivial_properties && isTrivialGetterOrSetter(md)) {
                if (verbose) {
                    System.out.printf(
                            "skipping trivial property method %s%n", md.getNameAsString());
                }
                return;
            }
    ...
    private static void getOrphanCommentsBeforeThisChildNode(
            final Node node, List<Comment> result) {
