...
public class RLCCalledMethodsAnnotatedTypeFactory extends CalledMethodsAnnotatedTypeFactory
     ...
     *
     * <p>Note that in an ideal world, this would be an {@code IdentityBiMap}: that is, a BiMap
     * using {@link java.util.IdentityHashMap} as both of the backing maps. However, Guava doesn't
     * have such a map AND their implementation is incompatible with IdentityHashMap as a backing
     * map, because even their {@code AbstractBiMap} class uses {@code equals} calls in its
     * implementation (and its documentation calls out that it and all its derived BiMaps are
     * incompatible with IdentityHashMap as a backing map for this reason). Therefore, we use a
     * regular BiMap. Doing so is safe iff 1) the LocalVariableNode keys all have different names,
     * and 2) a standard Tree implementation that uses reference equality for equality (e.g., JCTree
     * in javac) is used.
     */
    ...
    /**
     * Retrieves the {@code @MustCall} annotation for the given object, which can be either an
     * {@link Element} or a {@link Tree}. This method delegates to the {@code
     * MustCallAnnotatedTypeFactory} to get the annotated type of the input object and then extracts
     * the primary {@code @MustCall} annotation from it.
     *
     * @param obj the object for which to retrieve the {@code @MustCall} annotation. Must be either
     *     an instance of {@link Element} or {@link Tree}.
    ...
    /**
     * Returns whether the {@link MustCall#value} element/argument of the @MustCall annotation on
     * the type of {@code tree} is definitely empty.
     *
    ...
    /**
     * Returns whether the {@link MustCall#value} element/argument of the @MustCall annotation on
     * the type of {@code element} is definitely empty.
     *
     * <p>This method only considers the declared type: it does not consider flow-sensitive
     * refinement.
     *
        ...
        if (subCheckerClass == MustCallChecker.class) {
            if (!canCreateObligations()) {
                return super.getTypeFactoryOfSubcheckerOrNull(
                        MustCallNoCreatesMustCallForChecker.class);
            }
        }
    ...
    /**
     * Fetches the transfer input for the given block, either from the flowResult, if the analysis
     * is still running, or else from the analysis itself.
     *
     * @param block a block
