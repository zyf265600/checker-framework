    ...
    protected class UpperBoundTreeAnnotator extends TreeAnnotator {
        ...
        public Void visitMethodInvocation(MethodInvocationTree tree, AnnotatedTypeMirror type) {
            ...
            if (imf.isIndexOfString(tree)) {
                ...
                // the empty string: "".indexOf("") returns 0, which isn't an index into "". So,
                // this
                // special case modifies the return type of these methods if either the parameter or
                // the receiver is known (by the Value Checker) to not be the empty string. There
                // are
                // three ways the Value Checker might have that information: either string could
                // have a
                // @StringVal annotation whose value doesn't include "", either could have an
                // @ArrayLen
                // annotation
                // whose value doesn't contain zero, or either could have an @ArrayLenRange
                // annotation whose
                // from
