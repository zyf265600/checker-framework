...
public class NonEmptyTransfer extends CFTransfer {
    ...
    public TransferResult<CFValue, CFStore> visitCase(
        ...
        refineSwitchStatement(
                switchOpNode, caseOperands, result.getThenStore(), result.getElseStore());
    ...
    /**
     * Returns true if the receiver of {@code methodAccessNode} is non-empty according to {@code
     * in}.
     *
     * @param in used to get the type of {@code methodAccessNode}.
     * @param methodAccessNode method access
     * @return true if the receiver of {@code methodAccessNode} is non-empty according to {@code
     *     in}.
     */
     ...
     * expressions of the form {@code container.size() != n}, {@code n != container.size()}, or
     * {@code container.indexOf(Object) != n}.
     *
     * <p>This method is always called twice, with the arguments reversed. So, it can do
     * non-symmetric checks.
     *
     ...
     * {@code @NonEmpty}.
     *
     * <p>This method is also used to refine the "then" branch of an equality comparison where
     * {@code container.size()} is compared against a non-zero value.
     *
     * @param left the left operand of a binary operation
     * @param right the right operand of a binary operation
     * @param store the abstract store to update
    ...
    /**
     * Updates the transfer result's store with information from the Non-Empty type system for
     * switch statements, where the test expression is of the form {@code container.size()} or
     * {@code container.indexOf(Object)}.
     *
     * <p>For example, the "then" store of any case node with an integer value greater than 0 (or
     * -1, in the case of the test expression being a call to {@code container.indexOf(Object)})
     * should refine the type of {@code container} to {@code @NonEmpty}.
     *
    ...
    /**
     * Return true if the given node is an invocation of {@link java.util.Collection#size()} or
     * {@link java.util.Map#size()}.
     *
     * @param possibleSizeAccess a node that may be a method call to the {@code size()} method in
     *     the {@link java.util.Collection} or {@link java.util.Map} types
