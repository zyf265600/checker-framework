...
public class MustCallInference {
     ...
     *
     * <p>Operationally, it checks method invocations for fields with non-empty @MustCall
     * obligations along all paths to the regular exit point in the method body of the method
    ...
    /**
     * Updates {@code worklist} with the next block along all paths to the regular exit point. If
     * the next block is a regular exit point, adds an {@literal @}Owning annotation for fields in
     * {@link #owningFields}.
     *
    ...
    private List<Block> getNormalSuccessors(Block cur) {
        ...
        if (cur.getType() == Block.BlockType.CONDITIONAL_BLOCK) {
            ...
            if (!(cur instanceof SingleSuccessorBlock)) {
                throw new BugInCF(
                        "BlockImpl is neither a conditional block nor a SingleSuccessorBlock");
            }
