...
public class ResourceLeakVisitor extends CalledMethodsVisitor {
     ...
     * ResourceLeakAnnotatedTypeFactory implements but CalledMethodsAnnotatedTypeFactory does not.
     */
    ...
    */
    ...
    private void checkCreatesMustCallForTargetsHaveNonEmptyMustCall(
        ...
        List<JavaExpression> createsMustCallExprs =
                CreatesMustCallForToJavaExpression
                        .getCreatesMustCallForExpressionsAtMethodDeclaration(tree, mcAtf, mcAtf);
    ...
    /**
     * Given a constructor, a {@code @MustCallAlias} must appear in both the list of parameters and
     * as an annotation on the constructor itself, if it is to appear at all.
     *
     ...
     *
     * @param paramWithMustCallAliasAnno a parameter with a {@code @MustCallAlias} annotation, null
     *     if there are none
    ...
    private void checkMustCallAliasAnnoMismatch(
        ...
        if (isMustCallAliasAnnotationOnParameter != isMustCallAliasAnnoOnMethodOrConstructorDecl) {
            String locationOfCheck =
                    TreeUtils.isClassTree(tree) ? "this constructor" : "the return type";
            String message =
    ...
    */
    ...
    /**
     * Returns the {@link CreatesMustCallFor#value} element/argument of the
     * given @CreatesMustCallFor annotation, or "this" if there is none.
     *
    ...
    private static String getCreatesMustCallForValue(
        ...
        return AnnotationUtils.getElementValue(
                createsMustCallFor,
                mcAtf.getCreatesMustCallForValueElement(),
                String.class,
                "this");
    }

    /**
     * Returns all the {@link CreatesMustCallFor#value} elements/arguments of
     * all @CreatesMustCallFor annotations on the given element.
     *
     ...
     * type of {@code field} is {@code @MustCall("m"}}. This method checks that the enclosing class
     * of {@code field} has a type {@code @MustCall("m2")} for some method {@code m2}, and that
        ...
        if (enclosingMustCallValues == null) {
            ...
            for (Element siblingElement : siblingsOfOwningField) {
                if (siblingElement.getKind() == ElementKind.METHOD
                        && enclosingMustCallValues.contains(
                                siblingElement.getSimpleName().toString())) {
