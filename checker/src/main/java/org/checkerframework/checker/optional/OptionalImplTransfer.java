...
public class OptionalImplTransfer extends CFTransfer {
    ...
    public OptionalImplTransfer(CFAbstractAnalysis<CFValue, CFStore, CFTransfer> analysis) {
        ...
        streamReduceNoIdentity =
                TreeUtils.getMethodOrNull("java.util.stream.Stream", "reduce", 1, env);
                ...
                if (lambdaParent.getKind() == Tree.Kind.METHOD_INVOCATION) {
                    ...
                    if (methodElt.equals(optionalIfPresent)
                        ...
                        ExpressionTree methodSelectTree =
                                TreeUtils.withoutParens(invok.getMethodSelect());
                        ExpressionTree receiverTree =
                                ((MemberSelectTree) methodSelectTree).getExpression();
    ...
    private TransferResult<CFValue, CFStore> refineNonEmptyToPresentStreamResult(
        ...
        if (NodeUtils.isMethodInvocation(
                methodInvok,
                nonEmptyToPresentStreamMethods,
                optionalTypeFactory.getProcessingEnv())) {
            if (isReceiverParameterNonEmpty(methodInvok)) {
                ...
                CFValue value =
                        analysis.createSingleAnnotationValue(PRESENT, methodInvok.getType());
