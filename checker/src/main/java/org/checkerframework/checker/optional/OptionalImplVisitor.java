...
public class OptionalImplVisitor
     ...
     * visited. A method depends on the {@link NonEmpty} type system if any of the following is
     * true:
     *
     * <ul>
     ...
     * the caller of the method (i.e., the method that encloses the method invocation) is added to
     * the list of callers of the method being invoked (i.e., the callee) in {@link
     * calleesToCallers}.
     */
     ...
     * names (rather than fully-qualified names or signatures) is a bit imprecise, because it
     * includes all overloads.
     *
     * <p>This is not a complete mapping of <i>all</i> callees and callers in a program. It
     * comprises methods that have programmer-written annotations from the {@link NonEmpty} type
     * system, and their immediate dependents.
     *
     ...
     * callees). If it is found, then the corresponding values (i.e., the names of the callers of
     * the method being visited) are added to {@link namesOfMethodsToVerifyWithNonEmptyChecker}.
     */
    ...
    /**
     * Returns true iff the method being called is annotated with {@code @}{@link
     * OptionalPropagator}. This is the Optional propagation methods: filter, flatMap, map, or.
     *
     * @param methInvok a method invocation
    ...
    /**
     * Returns true iff the method being called is annotated with {@code @}{@link
     * OptionalEliminator}. This is the Optional elimination methods: get, orElse, orElseGet,
     * orElseThrow.
     *
     * @param methInvok a method invocation
     ...
     *
     * <p>This method would ideally be in {@link TreeUtils} as a public static method, but this
     * would introduce the {@code dataflow} package as a dependency in {@code javacutil}, which is
     * undesirable.
     *
    ...
    public void handleConditionalStatementIsPresentGet(IfTree tree) {
        ...
        if (isPresentReceiver instanceof MethodInvocationTree) {
            ExecutableElement ele =
                    TreeUtils.elementFromUse((MethodInvocationTree) isPresentReceiver);
            boolean isPure =
                    PurityUtils.isDeterministic(atypeFactory, ele)
                            && PurityUtils.isSideEffectFree(atypeFactory, ele);
            if (!isPure) {
     ...
     * </ul>
     *
     * inside the body of the {@code then} block for {@code VAR.isPresent()} or the {@code else}
     * block for {@code VAR.isEmpty()}.
     *
     ...
     * updated if the callee of the invocation is present in the map. That is, the map is updated
     * with the callers of any methods that have programmer-written annotations from the {@link
     * NonEmpty} system.
     *
     * <p>These annotations may appear on the return type, formal parameters, or on the declaration
     * of local variables within method bodies.
     *
    ...
    private boolean isAnnotatedWithNonEmptyPrecondition(MethodTree methodDecl) {
        List<? extends AnnotationMirror> annos =
                TreeUtils.annotationsFromTypeAnnotationTrees(
                        methodDecl.getModifiers().getAnnotations());
    ...
    private boolean isAnyFormalAnnotatedWithNonEmpty(MethodTree methodDecl) {
        ...
        for (VariableTree vt : params) {
            List<? extends AnnotationMirror> annos =
                    TreeUtils.annotationsFromTypeAnnotationTrees(
                            vt.getModifiers().getAnnotations());
    ...
    private boolean isReturnTypeAnnotatedWithNonEmpty(MethodTree methodDecl) {
        ...
        List<? extends AnnotationMirror> annos =
                TreeUtils.typeOf(returnType).getAnnotationMirrors();
    ...
    private void handleCompareToNull(BinaryTree tree) {
        ...
        if (leftOp.getKind() == Tree.Kind.NULL_LITERAL
                && isOptionalType(TreeUtils.typeOf(rightOp))) {
            checker.reportWarning(tree, "optional.null.comparison");
        }
        if (rightOp.getKind() == Tree.Kind.NULL_LITERAL
                && isOptionalType(TreeUtils.typeOf(leftOp))) {
            checker.reportWarning(tree, "optional.null.comparison");
        }
    }
     ...
     *
     * <p>This creation-propagation-elimination pattern can be eliminated by directly checking
     * whether a value is null before invoking methods on it.
     *
     ...
     *
     * <p>Certain types are illegal, such as {@code Optional<Optional>}. The type validator may see
     * a supertype of the most precise run-time type; for example, it may see the type as {@code
     ...
     *
     * @param tree a method invocation that might create {@code Optional<X>} where X is
     *     impermissable: Optional or Collection
     */
    ...
    private final class OptionalImplTypeValidator extends BaseTypeValidator {
         ...
         * Handles Rule #5, part of Rule #6, and also Rule #7.
         *
         * <p>Rule #5: Avoid nested Optional chains, or operations that have an intermediate
         * Optional value.
         *
         * <p>Rule #6: Don't use Optional in fields, parameters, and collections.
         *
    ...
    private boolean isFilterIsPresentMapGet(MemberReferenceTree memberRefTree) {
        ...
        if (getParent.getKind() == Tree.Kind.METHOD_INVOCATION) {
            ...
            if (mapReceiverTree != null
                    && mapReceiverTree.getKind() == Tree.Kind.METHOD_INVOCATION) {
