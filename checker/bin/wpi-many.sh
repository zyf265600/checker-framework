...
while IFS='' read -r line || [ "$line" ]; do
  ...
  mkdir -p "./${REPO_NAME_HASH}" || {
    echo "command failed in $(pwd): mkdir -p ./${REPO_NAME_HASH}"
    exit 5
  }

  cd "./${REPO_NAME_HASH}" || {
    echo "command failed in $(pwd): cd ./${REPO_NAME_HASH}"
    exit 5
  }
  ...
  if [ ! -d "${REPO_NAME}" ]; then
    ...
    mkdir "./${REPO_NAME}" || {
      echo "command failed in $(pwd): mkdir ./${REPO_NAME}"
      exit 5
    }
    cd "./${REPO_NAME}" || {
      echo "command failed in $(pwd): cd ./${REPO_NAME}"
      exit 5
    }
    git init
  ...
  git checkout "${HASH}" || {
    echo "command failed in $(pwd): git checkout ${HASH}"
    exit 5
  }
