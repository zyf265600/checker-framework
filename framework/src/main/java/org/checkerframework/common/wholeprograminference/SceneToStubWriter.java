...
public final class SceneToStubWriter {
     ...
     */
    @SuppressWarnings(
            "signature:return") // A binary name without its package is still a binary name
    ...
    /**
     * Prints all annotations in {@code annos} to {@code sb} in a form suitable to be printed as
     * Java source code.
     *
     * <p>Each annotation is followed by a space, to separate it from following Java code.
     *
     ...
     * @param annos the annotations to format
     */
    private static void formatAnnotations(
            StringBuilder sb, Collection<? extends Annotation> annos) {
     ...
     * annotations from the scenelib representation added in appropriate places. The result includes
     * a trailing space.
     *
        ...
        try {
            ...
            for (String clazz : classes) {
                if (isPrintable(clazz, scene.getAScene().getClasses().get(clazz))) {
                    ...
                    printClass(
                            clazz, scene.getAScene().getClasses().get(clazz), checker, printWriter);
                }
            }
        } finally {
            if (printWriter != null) {
