...
public class WholeProgramInferenceImplementation<T> implements WholeProgramInference {
    ...
    /**
     * Whether to print debugging information when an inference is attempted, but cannot be
     * completed. An inference can be attempted without success for example because the current
     * storage system does not support placing annotation in the location for which an annotation
     * was inferred.
     */
    ...
    private final String SIDE_EFFECT_FREE_NAME =
                ...
                if (expandArgATM) {
                    ...
                    AnnotatedTypeMirror argArray =
                            AnnotatedTypeMirror.createType(
                                    TypesUtils.createArrayType(
                                            argATM.getUnderlyingType(), atypeFactory.types),
                                    atypeFactory,
                                    false);
    ...
    public void updateContracts(
        ...
        if (!ElementUtils.isStatic(methodElt)) { // Static methods do not have a receiver.
            ...
            if (v != null) {
                ...
                AnnotatedTypeMirror inferredType =
                        AnnotatedTypeMirror.createType(
                                declaredType.getUnderlyingType(), atypeFactory, false);
     ...
     * "lattice", Impure is the top, SideEffectFree and Deterministic are siblings below it, and
     * Pure is the bottom, below them. Note that this routine is "fail-safe": Impure is returned if
     * either of the input annotations is not actually a purity annotation.
     *
    ...
    /**
     * Returns the purity annotation ({@link Pure}, {@link SideEffectFree}, {@link Deterministic},
     * or {@link Impure}) currently associated with the given executable element in this round of
    ...
    public void updateAtmWithLub(AnnotatedTypeMirror sourceCodeATM, AnnotatedTypeMirror ajavaATM) {
        ...
        switch (sourceCodeATM.getKind()) {
            ...
            case ARRAY:
                ...
                AnnotatedTypeMirror ajavaComponent =
