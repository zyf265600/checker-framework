...
class ValueTypeAnnotator extends TypeAnnotator {
     ...
     * {@link ValueVisitor#visitAnnotation(com.sun.source.tree.AnnotationTree, Void)} which issues
     * warnings to users in these cases.
     *
     ...
     * {@link ValueVisitor#visitAnnotation(com.sun.source.tree.AnnotationTree, Void)} raises an
     * error to users if the annotation was user-written.
     *
     * <p>If any @ArrayLen annotation has a negative number, replaces the annotation by {@code
     * BottomVal}. The {@link ValueVisitor#visitAnnotation(com.sun.source.tree.AnnotationTree,
     ...
     * computes an appropriate default based on the underlying type for the other side of the range.
     * For instance, if the user writes {@code @IntRange(from = 1) short x;} then this method will
