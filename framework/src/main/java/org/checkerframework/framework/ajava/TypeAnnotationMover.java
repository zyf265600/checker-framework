...
public class TypeAnnotationMover extends VoidVisitorAdapter<Void> {
     ...
     * @param declarationType the type of declaration {@code node} represents; always FIELD or
     *     METHOD
     ...
     * @return true unless {@code annotation} definitely cannot be a declaration annotation for
     ...
     *
     * <p>Annotations should not be moved onto a Type for which this method returns true. A type
     * like {@code @Anno java.lang.String} is illegal since the annotation should go directly next
     * to the rightmost part of the fully qualified name, like {@code java.lang. @Anno String}. So
     * if a file contains a declaration like {@code @Anno java.lang.String myField}, the annotation
     * must belong to the declaration and not the type.
     *
     * <p>If a declaration contains an inner class type like {@code @Anno Outer.Inner myField}, it
     * may be the case that {@code @Anno} belongs to the type {@code Outer}, not the declaration,
     * and should be moved, but it's impossible to distinguish this from the above case using only
     * the JavaParser AST for a file. To be safe, the annotation still shouldn't be moved, but this
     * may lead to suboptimal formatting placing {@code @Anno} on its own line.
     *
