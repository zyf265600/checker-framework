...
public class DefaultTypeHierarchy extends AbstractAtmComboVisitor<Boolean, Void>
     ...
     *
     * @param inside a possibly-contained type; its underlying type is contained by {@code
     *     outside}'s underlying type
    ...
    protected boolean visitTypeArgs(
        ...
        if (subtypeTypeArgs.size() != supertypeTypeArgs.size()) {
            throw new BugInCF(
                    "Type arguments are not the same size: %s %s", subtypeAsSuper, supertype);
        }
        ...
        AnnotationMirror covariantAnno =
                typeFactory.getDeclAnnotation(supertypeElem, Covariant.class);
    ...
    public Boolean visitTypevar_Typevar(
        ...
        if (AnnotatedTypes.haveSameDeclaration(checker.getTypeUtils(), subtype, supertype)) {
            ...
            boolean supertypeHasAnno =
                    supertype.getPrimaryAnnotationInHierarchy(currentTop) != null;
    ...
    /**
     * If {@code type} is a type variable or wildcard recur on its upper bound until an upper bound
     * is found that is neither a type variable nor a wildcard.
     *
     * @param type the type
     * @return if {@code type} is a type variable or wildcard, recur on its upper bound until an
     *     upper bound is found that is neither a type variable nor a wildcard. Otherwise, return
     *     {@code type} itself.
     */
