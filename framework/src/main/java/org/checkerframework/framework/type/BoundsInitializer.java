...
public class BoundsInitializer {
    ...
    /**
     * A class that creates an {@link AnnotatedTypeMirror} to match a TypeMirror. This visitor is
     * only used to initialize recursive type variables or wildcards, because at some point instead
     * of creating a new type, a previously created type is returned. This makes the {@code
    ...
    private static class BoundInitializerVisitor implements TypeVisitor<AnnotatedTypeMirror, Void> {
        ...
        /**
         * A map from a Java type variable in a raw type to an {@link AnnotatedWildcardType}. Used
         * to set up recursive type variables.
         */
        private final Map<TypeVariable, AnnotatedWildcardType> typeParamToWildcard =
                new HashMap<>();
         ...
         * AnnotatedTypeMirror} that match the upper and lower bounds of the underlying type of
         * {@code annotatedTypeVariable} by visiting each bound. This method should only be called
         * once per {@link TypeVariable}.
         *
        ...
        private void initializeTypeVariable(AnnotatedTypeVariable annotatedTypeVariable) {
            ...
            TypeMirror lowerBound =
                    TypesUtils.getTypeVariableLowerBound(t, atypeFactory.processingEnv);
         ...
         * AnnotatedTypeMirror} that match the upper and lower bounds of the underlying type of
         * {@code annotatedWildcardType} by calling visiting each bound. This method should only be
         * called once per {@link WildcardType}.
         *
        ...
        public AnnotatedTypeMirror visitDeclared(DeclaredType t, Void unused) {
            AnnotatedDeclaredType annotatedDeclaredType =
                    (AnnotatedDeclaredType) createAnnotatedType(t);
            if (t.getEnclosingType() != null
                    && t.getEnclosingType().getKind() == TypeKind.DECLARED) {
                annotatedDeclaredType.setEnclosingType(
                        (AnnotatedDeclaredType) visit(t.getEnclosingType()));
            }

            TypeElement typeElement = (TypeElement) atypeFactory.types.asElement(t);
            List<AnnotatedTypeMirror> typeArgs =
                    new ArrayList<>(typeElement.getTypeParameters().size());
                ...
                for (TypeParameterElement typeParameterEle : typeElement.getTypeParameters()) {
                    ...
                    if (wildcardType == null) {
                        TypeMirror javaTypeArg =
                                getUpperBoundAsWildcard(typeVar, atypeFactory.types);
        ...
        public AnnotatedTypeMirror visitUnion(UnionType t, Void unused) {
            ...
            annotatedUnionType.alternatives =
                    CollectionsPlume.mapList(
                            alternative -> (AnnotatedDeclaredType) visit(alternative),
                            t.getAlternatives());
