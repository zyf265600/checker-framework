...
public class DefaultTypeArgumentInference implements TypeArgumentInference {
    ...
    public InferenceResult inferTypeArgs(
        ...
        if (outerTree != expressionTree) {
            if (outerTree.getKind() == Tree.Kind.METHOD_INVOCATION) {
                ...
                throw new BugInCF(
                        "Unexpected kind of outer expression to infer type arguments: %s",
                        outerTree.getKind());
            }
        } else {
    ...
    public static ExpressionTree outerInference(
            ExpressionTree tree, @Nullable TreePath parentPath) {
        if (parentPath == null) {
            return tree;
        }
        ...
        switch (parentTree.getKind()) {
            ...
            case NEW_CLASS:
                ...
                if (argumentNeedsInference(
                        constructor, newClassTree.getArguments(), tree, newClassTree)) {
                    return outerInference((ExpressionTree) parentTree, parentPath.getParentPath());
                }
                return tree;
            case RETURN:
    ...
    /**
     * Returns true if {@code argTree} is pseudo-assigned to a parameter in {@code
    ...
    private static boolean argumentNeedsInference(
        ...
        if (executableElement.getKind() == ElementKind.CONSTRUCTOR) {
            ...
            if (newClassTree != null && TreeUtils.isDiamondTree(newClassTree)) {
                DeclaredType declaredType =
                        (DeclaredType)
                                ((DeclaredType) TreeUtils.typeOf(newClassTree))
                                        .asElement()
                                        .asType();
        ...
        return ContainsInferenceVariable.hasAnyTypeVariable(
                executableType.getTypeVariables(), param);
    }
}
