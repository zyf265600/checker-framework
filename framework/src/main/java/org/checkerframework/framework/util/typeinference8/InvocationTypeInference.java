    ...
    public InferenceResult infer(ExpressionTree invocation, AnnotatedExecutableType methodType)
        ...
        Theta map =
                context.inferenceTypeFactory.createThetaForInvocation(
                        invocation, invocationType, context);
    ...
    public BoundSet createB3(
        ...
        if (b2.isUncheckedConversion()) {
            ...
            BoundSet b =
                    new ConstraintSet(
                                    new Typing(
                                            source, r.getErased(), target, Kind.TYPE_COMPATIBILITY))
                            .reduce(context);
            ...
            BoundSet b =
                    CaptureBound.createAndIncorporateCaptureConstraint(
                            r, target, invocation, context);
            b2.incorporateToFixedPoint(b);
            return b2;
            ...
            if (target.isProper() && target.getJavaType().getKind().isPrimitive()) {
                ...
                String source =
                        "Constraint between method call type and target type for method call: "
                                + invocation;
            ...
            case NEW_CLASS:
                if (TreeUtils.isPolyExpression(expression)) {
                    try {
                        ...
                        // inference
                        // variables must be resolved first. This happens when a lambda parameter is
                        // used in the
        ...
        switch (expressionTree.getKind()) {
            case LAMBDA_EXPRESSION:
                ...
                if (TreeUtils.isImplicitlyTypedLambda(lambda)
                        || formalParameterType.isUseOfVariable()) {
            ...
            case CONDITIONAL_EXPRESSION:
                ...
                // applicability.
                return notPertinentToApplicability(
                                conditional.getTrueExpression(), formalParameterType)
                        || notPertinentToApplicability(
                                conditional.getFalseExpression(), formalParameterType);
            default:
    ...
    private BoundSet getB4(BoundSet b3, ConstraintSet c) {
        ...
        while (!c.isEmpty()) {
            ...
            ConstraintSet subset =
                    ConstraintSet.getClosedSubset(c, b3.getDependencies(newVariables));
