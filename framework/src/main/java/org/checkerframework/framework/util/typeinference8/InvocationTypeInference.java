    ...
    public BoundSet createB3(
        ...
        if (b2.isUncheckedConversion()) {
            ...
            if (target.isProper() && target.getJavaType().getKind().isPrimitive()) {
                ...
                ConstraintSet constraintSet =
                        new ConstraintSet(new Typing(r, target, Kind.SUBTYPE));
                BoundSet newBounds = constraintSet.reduce(context);
    ...
    private BoundSet getB4(BoundSet b3, ConstraintSet c) {
        ...
        while (!c.isEmpty()) {
            ...
            if (!subset.isEmpty()) {
                ...
                // found.
                // In this case, a new subset needs to be picked so that dependencies of the
                // constraints from reducing the additional argument constraint can be taken into
                // account.
                c.addAll(subset);
            }
