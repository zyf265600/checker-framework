...
public class ConstraintSet implements ReductionResult {
    ...
    /**
     * The Java types are correct, but the qualifiers are not in the correct relationship. Return
     * this rather than throwing an exception so that type arguments with the correct Java type are
     * still inferred.
     */
    @SuppressWarnings("interning:assignment")
     ...
     * influence an output variable of another constraint in C. If that subset is empty, returns a
     * set containing a single constraint that participates in a constraint cycle. (See JLS
     * 18.5.2.2)
     *
     * @param c a constraint set
     * @param dependencies an object describing the dependencies of inference variables
     * @return s a subset of constraints in {@code c} whose inputs do not affect {@code c}'s
     *     outputs, or a singleton constraint from a constraint cycle
     */
    public static ConstraintSet getClosedSubset(ConstraintSet c, Dependencies dependencies) {
        ...
        // inference variable gamma such that alpha can influence gamma and gamma can influence
        // beta.

        // Put another way:
        // Find a subset of the set c where the following is true for all the constraints in the
        // subset:
        // no input variable of a constraint can influence an output variable of any constraint in
        // c.
        ...
        if (!subset.isEmpty()) {
            // Return the first expression constraint; if there are none, return the first
            // constraint.
        ...
        // checker/tests/all-systems/java8inference/MapEntryGetFails.java is a test that uses this
        // code.
