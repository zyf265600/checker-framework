...
public class VariableBounds {
    ...
    /**
     * Qualifier bounds on this variable. Stored as a map from kind of bound (upper, lower, equal)
     * to a set of {@link AnnotationMirror}s. A qualifier bound is a bound on the primary annotation
     * of this variable.
     */
    ...
    private void addQualifierConstraint(
            Set<? extends AbstractQualifier> setT,
            Set<? extends AbstractQualifier> setS,
            Kind kind) {
    ...
    public void addConstraintsFromComplementaryBounds(BoundKind kind, AbstractType boundType) {
        ...
        if (boundType.isUseOfVariable()) {
            ...
            switch (kind) {
                case EQUAL:
                    boundVar.addQualifierBound(
                            BoundKind.EQUAL, qualifierBounds.get(BoundKind.EQUAL));
                    boundVar.addQualifierBound(
                            BoundKind.LOWER, qualifierBounds.get(BoundKind.LOWER));
                    boundVar.addQualifierBound(
                            BoundKind.UPPER, qualifierBounds.get(BoundKind.UPPER));
                    break;
                case LOWER:
                    boundVar.addQualifierBound(
                            BoundKind.UPPER, qualifierBounds.get(BoundKind.EQUAL));
                    boundVar.addQualifierBound(
                            BoundKind.LOWER, qualifierBounds.get(BoundKind.LOWER));
                    break;
                case UPPER:
                    boundVar.addQualifierBound(
                            BoundKind.LOWER, qualifierBounds.get(BoundKind.EQUAL));
                    boundVar.addQualifierBound(
                            BoundKind.UPPER, qualifierBounds.get(BoundKind.UPPER));
                    break;
            }
     ...
     * subtype of {@code G<...>}, but S is a subtype of the raw type {@code |G<...>|}, where {@code
     * G} a generic class or interface for which the parameter of this method, {@code t}, is a
     * parameterization.
     *
