...
public class UseOfVariable extends AbstractType {
    ...
    public AbstractType create(
        ...
        return InferenceType.create(
                atm, type, variable.map, qualifierVars, context, ignoreAnnotations);
    }
    ...
    public void addBound(Constraint parent, BoundKind kind, AbstractType bound) {
        if (!hasPrimaryAnno) {
            ...
            // If the use has a primary annotation, then mark the bound so that the annotations will
            // be ignored. Also, set to bottom or top, unless the bound is a type variable. This way
            // if all the bounds of a variable have annotations to be ignored, the instantiation of
            // that variable is as flexable as possible.
