...
public class CaptureBound {
     ...
     * @param invocation invocation a method or constructor invocation; used to create fresh
     *     inference variables
    ...
    /**
     * Given {@code r}, a parameterized type, {@code G<A1, ..., An>}}, and one of {@code A1, ...,
     * An} is a wildcard, then, for fresh inference variables {@code B1, ..., Bn}, the constraint
     * formula {@code <G<B1, ..., Bn> -> T>} is reduced and incorporated, along with the bound
     * {@code G<B1, ..., Bn> = capture(G<A1, ..., An>)}, with B2.
     *
    ...
    private BoundSet incorporate(AbstractType target, Java8InferenceContext context) {
        ...
        for (CaptureTuple t : tuples) {
            if (t.capturedTypeArg.getTypeKind() != TypeKind.WILDCARD) {
                ...
                t.alpha
                        .getBounds()
                        .addBound(null, VariableBounds.BoundKind.EQUAL, t.capturedTypeArg);
            }
        }

        String source = "Captured constraint from method call: " + invocation;
        ConstraintSet set =
                new ConstraintSet(new Typing(source, lhs, target, Kind.TYPE_COMPATIBILITY));
