...
programming language.  The Dataflow Framework is used in the
...
\\
The main class is \code{org.checkerframework.dataflow.cfg.playground.BusyExpressionPlayground}.
...
if they are nested \code{BinaryOperationNode}s. We then use \code{putBusyExpr(BusyExprValue)}
...
%% Problem: Emacs function `hack-local-variables` only looks backwards 3000
%% characters for the "Local Variables" block, that number is hardcoded,
%% and the "LocalWords" part of this file is longer than that if its lines
%% are only (say) 100 characters long.
...
%  LocalWords: TernaryExpression UnsignedRightShift NotEqual GreaterThan GreaterThanOrEqual LessThan LessThanOrEqual ArrayCreation StringConcatenateAssignment ObjectCreation TypeCast cond
