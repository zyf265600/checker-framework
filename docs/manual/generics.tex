...
Section~\ref{creating-declarative-hierarchy} explains how to define a
...
\end{Verbatim}
Method \<m2b()> can be called on (for example) \code{List<@Nullable
  MySubClass>} and \code{List<@NonNull MySubClass>}, which are not legal
...
% LocalWords:  PolyNull java lub invariantly supertype's
