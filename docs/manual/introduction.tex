...
\item
  ...
  \\
  %END LATEX
  \url{https://checkerframework.org/checker-framework-3.28.0-eisop1.zip}
...
\item \<-AwarnRedundantAnnotations>
  This flag warns about redundant annotations.
  A warning is issued if the explicitly written annotation
  on a type is the same as the default annotation for this type and location.
...
\item \<-ApermitUnsupportedJdkVersion>
  \label{permitUnsupportedJdkVersion}
  Issue a note, rather than a warning, when running the Checker Framework
  on a JDK version that the Checker Framework maintainers do not support.
  ...
  Also see the \<-AnoJreVersionCheck> option below.
\item \<-AnoJreVersionCheck>
  Completely disable the JDK version check and issue neither a warning nor
  a note. This is useful to allow Checker Framework tests to run on any
  JDK, but could also be useful when the warning/note is redundant for a
  user.
  See comments for \<-ApermitUnsupportedJdkVersion> option above.
