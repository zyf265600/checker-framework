...
\ahreforurl{https://htmlpreview.github.io/?https://github.com/eisop/checker-framework/master/docs/developer/developer-manual.html}{Checker
...
\myurl{https://eisop.github.io/cf/api/}.
...
\multicolumn{2}{p{3\bw}|}{\centering \href{https://eisop.github.io/afu/}{Annotation File Utilities} \par (\code{.java} $\leftrightarrow$ \code{.class} files)}
\\ \hline
...
%       "root" for ASTs, in particular for CompilationUnitTrees.
  ...
  will use \code{@SubtypeOf(Unqualified.class)}.  For an example, see the
...
\<NullType> is the bottom type in the Java type hierarchy, and it is a special unnamed type i.e. programmers cannot write
\<NullType> explicitly in the source code.
\<null> has the bottom type. Because the only value with type \<Void> is \<null>, uses of the type \<Void> are bottom.
(The only exception is if the type system has special treatment for \<null> values, as the
...
and \refqualclasswithparams{framework/qual}{DefaultFor}{types = Void.class}
...
\subsectionAndLabel{Recursion in \<TreeAnnotator>s}{creating-procedurally-recursion-in-treeannotators}
...
\href{https://eisop.github.io/cf/manual/checker-framework-dataflow-manual.pdf}{Dataflow
  Manual}.
...
\item
  ...
  \url{https://github.com/eisop/jdk}.
...
\href{https://github.com/eisop/checker-framework/blob/master/checker/tests/README}{\<checker-framework/checker/tests/README>}.
...
qualifiers meta-annotated with
\refqualclass{framework/qual}{InvisibleQualifier}, which are
usually not shown.
...
being made), get a traceback (stack trace) from each Java thread,
...
\noindent
Note that you can also convert the \<.dot> file to various other file formats using
the \code{dot} program. The full list of supported file types can be found \href{http://www.graphviz.org/doc/info/output.html}{here}.
...
method \code{test} in the class \code{MyClass} in the project directory. If you run into
issues with the \code{com.sun} packages not being exported for JDK 11+, add the \code{---add-exports} flags
found in Section~\ref{javac-jdk11-non-modularized} to the above command.

\noindent To generate a string representation of the graph to standard output,
