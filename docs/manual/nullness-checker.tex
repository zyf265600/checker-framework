...
the default in most locations.  All of the annotations are explained in
Section~\ref{nullness-annotations}.
See Section~\ref{writing-nullness-annotations} for details on how to
use the annotations and Section~\ref{generics} for details on
generics.
...
The Nullness Checker is actually an ensemble of three pluggable
...
\sectionAndLabel{What the Nullness Checker guarantees}{nullness-checks}

% TODO: The actual guarantee is a bit more subtle than this; put it here
% rather than this more imprecise one.
...
\item
  ...
  As a special case of an \refqualclass{checker/nullness/qual}{NonNull}
...
\item
  ...
  If you supply the \code{-AassumeInitialized} command-line option, then the
  checker will unsoundly assume that every field is initialized before
  being used. It will not do any checking of
  \refqualclass{checker/initialization/qual}{Initialized} and related qualifiers.

\item
  \label{nullness-conservativeArgumentNullnessAfterInvocation}%
  \label{nullness-invocationPreservesArgumentNullness}%
  If you supply the \code{-AconservativeArgumentNullnessAfterInvocation} command-line option, the soundness of
...
\item
  ...
  If you supply the \code{-AcheckEnclosingExpr} command-line option, the soundness of
...
\item[\refqualclass{checker/nullness/qual}{EnsuresNonNull}]
  ...
  With \<EnsuresNonNull>, the successful return (i.e., a non-exceptional
  return) of the annotated method results in the given expressions being
  non-null. See the Javadoc for examples of its use.
...
\item[\refqualclass{checker/initialization/qual}{PolyInitialized}]
\end{description}
  ...
  \begin{itemize}
    \item the argument has length 0:
      ...
      \begin{itemize}
        ...
        \item an array creation tree of size 0, e.g. \<c.toArray(new String[0])>.

        \item a reference to a private, final field with a zero-length array as initializer, e.g. \<c.toArray(EMPTY\_STRING\_ARRAY)>, where \<EMPTY\_STRING\_ARRAY> is initialized to \<new String[0]> (or an equivalent zero-length array expression).
      \end{itemize}
...
% This URL no longer works.  As of June 2022, the last Wayback Machine crawl
% was August 2019.
% \item
...
To suppress warnings from the Nullness, Initialization, and KeyFor Checkers,
use the \<@SuppressWarnings("nullness")> annotation.

To suppress warnings from the Nullness and Initialization Checkers without suppressing warnings
from the KeyFor Checker (see Section~\ref{map-key-checker}),
use the \<@SuppressWarnings("nullnessinitialization")> annotation.

To suppress warnings from the Nullness and KeyFor Checkers without suppressing warnings
from the Initialization Checker (see Section~\ref{initialization-checker}),
use the \<@SuppressWarnings("nullnesskeyfor")> annotation.
\<@SuppressWarnings("nullnessnoinit")> has the same effect as \<@SuppressWarnings("nullnesskeyfor")>.

To suppress warnings only from the Nullness Checker, use the \<@SuppressWarnings("nullnessonly")> annotation.

To suppress warnings only from the Initialization Checker, use the \<@SuppressWarnings("initialization")> annotation.

To suppress warnings only from the KeyFor Checker, use the \<@SuppressWarnings("keyfor")> annotation.
...
\subsectionAndLabel{Conservative nullness annotations on the JDK}{nullness-jdk-conservative}
...
\subsectionAndLabel{Publications}{nullness-publications}

The papers ``Practical pluggable types for Java''~\cite{PapiACPE2008}
(ISSTA 2008,
\myurl{https://homes.cs.washington.edu/~mernst/pubs/pluggable-checkers-issta2008.pdf})
...
\ahreforurl{https://eisop.github.io/cf/tutorial/}{Checker
...
% and with variables NONNULL_ANNOTATIONS and BASE_COPYABLE_ANNOTATIONS in
...
\hline
 ...
 ~android.support.annotation.RecentlyNonNull~ \\ \hline
 ~androidx.annotation.NonNull~ \\ \hline
 ~androidx.annotation.RecentlyNonNull~ \\ \hline
 ~com.android.annotations.NonNull~ \\ \hline
 ~com.google.firebase.database.annotations.NotNull~ \\ \hline
 ~com.google.firebase.internal.NonNull~ \\ \hline
 ~com.mongodb.lang.NonNull~ \\ \hline
 ~com.sun.istack.NotNull~ \\ \hline
 ~com.sun.istack.internal.NotNull~ \\ \hline
 ~com.unboundid.util.NotNull~ \\ \hline
 ~edu.umd.cs.findbugs.annotations.NonNull~ \\ \hline
 ~io.micrometer.core.lang.NonNull~ \\ \hline
 ~io.micronaut.core.annotation.NonNull~ \\ \hline
 ~io.reactivex.annotations.NonNull~ \\ \hline
 ~io.reactivex.rxjava3.annotations.NonNull~ \\ \hline
 ~jakarta.annotation.Nonnull~ \\ \hline
 ...
 ~libcore.util.NonNull~ \\ \hline
 ~lombok.NonNull~ \\ \hline
 ~net.bytebuddy.agent.utility.nullability.NeverNull~ \\ \hline
 ~net.bytebuddy.utility.nullability.NeverNull~ \\ \hline
 ~org.antlr.v4.runtime.misc.NotNull~ \\ \hline
 ...
 ~org.chromium.build.annotations.OptimizeAsNonNull~ \\ \hline
 ...
 ~org.eclipse.lsp4j.jsonrpc.validation.NonNull~ \\ \hline
...
\caption{Correspondence between other nullness annotations and the
  Checker Framework's \<NonNull> annotation.}
\label{fig-nullness-refactoring-nonnull}
\end{figure}
...
\hline
 ...
 ~android.support.annotation.RecentlyNullable~ \\ \hline
 ~androidx.annotation.Nullable~ \\ \hline
 ~androidx.annotation.RecentlyNullable~ \\ \hline
 ~com.android.annotations.Nullable~ \\ \hline
 ~com.beust.jcommander.internal.Nullable~ \\ \hline
 ~com.google.api.server.spi.config.Nullable~ \\ \hline
 ~com.google.firebase.database.annotations.Nullable~ \\ \hline
 ~com.google.firebase.internal.Nullable~ \\ \hline
 ~com.google.gerrit.common.Nullable~ \\ \hline
 ~com.google.protobuf.Internal.ProtoMethodAcceptsNullParameter~ \\ \hline
 ~com.google.protobuf.Internal.ProtoMethodMayReturnNull~ \\ \hline
 ~com.mongodb.lang.Nullable~ \\ \hline
 ~com.sun.istack.Nullable~ \\ \hline
 ~com.sun.istack.internal.Nullable~ \\ \hline
 ~com.unboundid.util.Nullable~ \\ \hline
 ...
 ~io.micrometer.core.lang.Nullable~ \\ \hline
 ~io.micronaut.core.annotation.Nullable~ \\ \hline
 ~io.reactivex.annotations.Nullable~ \\ \hline
 ~io.reactivex.rxjava3.annotations.Nullable~ \\ \hline
 ~io.vertx.codegen.annotations.Nullable~ \\ \hline
 ~jakarta.annotation.Nullable~ \\ \hline
 ~javax.annotation.CheckForNull~ \\ \hline
 ~javax.annotation.Nullable~ \\ \hline
 ~junitparams.converters.Nullable~ \\ \hline
 ~libcore.util.Nullable~ \\ \hline
 ~net.bytebuddy.agent.utility.nullability.AlwaysNull~ \\ \hline
 ~net.bytebuddy.agent.utility.nullability.MaybeNull~ \\ \hline
 ~net.bytebuddy.agent.utility.nullability.UnknownNull~ \\ \hline
 ~net.bytebuddy.utility.nullability.AlwaysNull~ \\ \hline
 ~net.bytebuddy.utility.nullability.MaybeNull~ \\ \hline
 ~net.bytebuddy.utility.nullability.UnknownNull~ \\ \hline
 ~org.apache.avro.reflect.Nullable~ \\ \hline
 ~org.apache.cxf.jaxrs.ext.Nullable~ \\ \hline
 ~org.apache.shindig.common.Nullable~ \\ \hline
 ...
 ~org.chromium.build.annotations.Nullable~ \\ \hline
 ...
 ~reactor.util.annotation.Nullable~ \\ \hline
\end{tabular}
&
$\Rightarrow$
...
%END LATEX
\caption{Correspondence between other nullness annotations and the
  Checker Framework's \<Nullable> annotation.}
...
not, if you are using an unannotated library (including an unannotated
...
% \href{https://findbugs.sourceforge.net/api/edu/umd/cs/findbugs/annotations/Nullable.html}{Javadoc};
...
the \code{-AassumeInitialized} command-line option.
...
% \urldef{\jlsconstructorbodyurl}{\url}{https://docs.oracle.com/javase/specs/jls/se17/html/jls-8.html#jls-8.8.7}
...
MyFile.java:123: error: call to initHelper() not allowed on the given receiver.
    initHelper();
              ^
  found   : @UnderInitialization(com.google.Bar.class) MyClass
  required: @Initialized MyClass
\end{Verbatim}
...
MyClass.java:7: error: call to helperMethod() not allowed on the given receiver.
    helperMethod();
                ^
  found   : @UnderInitialization(MyClass.class) MyClass
  required: @Initialized MyClass
1 error
\end{Verbatim}
...
To suppress both warnings related to nullness and warnings related to initialization,
...
To suppress only warnings related to initialization, use the
...
You can also disable initialization checking completely while retaining
nullness checking by running the Nullness Checker with the
\code{-AassumeInitialized} command-line option.
...
% LocalWords:  noInitForMonotonicNonNull UnknownInitialization isArray jdk
...
% LocalWords:  unclassified'' trustArrayLenZero ArrayLen toArray Java''
% LocalWords:  ConcurrentHashMap permitClearProperty setProperties
% LocalWords:  clearProperty AassumeKeyFor AassumeInitialized WeakReference checkers''
% LocalWords:  AinvocationPreservesArgumentNullness SpotBugs's
